        ...
        <table class="dataTable table table-stripped" style="width: 100%;">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $product->price }}</td>
                    <td>
                        <span class="right badge badge-{{ $product->status ? 'success' : 'danger' }}">{{ $product->status ? 'Available' : 'Unavailable' }}</span>
                    </td>
                    <td>{{ $product->created_at->format('Y-m-d h:ia') }}</td>
                    <td>{{ $product->updated_at->format('Y-m-d h:ia') }}</td>
                    <td>
                        <a class="btn btn-primary" href="{{ route('products.details', ['product_id' => $product->id]) }}">{{ __('Detail') }}</a>
                    </td>
                </tr>
                @endforeach
            </tbody>
